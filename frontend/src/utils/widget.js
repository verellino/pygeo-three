...
const widgetProcessor = {
  ...
  "v-select": (widget) => {
    return {
      ...
      "item-text": "label",
      "item-value": "value",
      "small-chips": widget.smallChips || true,
      multiple: widget.multiple,
      dense: true,
      widgetName: "v-autocomplete",
      clearable: true,
      itemText: widget.itemText ? widget.itemText : "label",
      itemValue: widget.itemValue ? widget.itemValue : null
    };
  },
  //单选控件
  "v-radio-group": (widget) => {
    return {
      ...widget,
      key: widget.key,
      label: widget.text,
  ...
  "v-text-field": (widget) => {
    return {
      ...
      readonly: widget.readonly//是否显示输入历史
    };
  },
  //下拉框控件
  // "v-select": (widget) => {
  ...
  //     label: widget.text,
  //     dense: true,
  //     items: widget.items ? widget.items : null,
  //     itemText: widget.itemText ? widget.itemText : null,
  //     itemValue: widget.itemValue ? widget.itemValue : null
  //   }
  // },
  //文办控件
...
export const validatorAdapter = (editField) => {
  const validators = {
    //必填项
    required: required(editField.widget.text),
    //手机号码
    phone: function (v) {
